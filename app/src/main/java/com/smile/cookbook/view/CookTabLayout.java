 ...
 */
public class CookTabLayout extends LinearLayout{
    /*默认的页卡颜色*/
    ...
    public CookTabLayout(Context context) {
        this(context, null);
    }

    public CookTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CookTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array=getResources().obtainAttributes(attrs, R.styleable.CookTabLayout);
        int n=array.getIndexCount();
        for(int i=0;i<n;i++){
            int attr=array.getIndex(i);
            switch (attr){
                case R.styleable.CookTabLayout_indicatorColor:
                      /*获取页卡颜色值*/
                    mIndicatorColor = array.getColor(attr, DEFAULT_INDICATOR_COLOR);
                    break;
                case R.styleable.CookTabLayout_dividerColor:
                    //获取分割线的颜色
                    mDividerColor=array.getColor(attr,DEFAULT_DIVIDER_COLOR);
                    break;
                case R.styleable.CookTabLayout_bottomLineColor:
                    //获取底部线的颜色
                    mBottomLineColor=array.getColor(attr,DEFAULT_BOTTOM_LINE_COLOR);

                    break;
                case R.styleable.CookTabLayout_bottomLineHeight:
                    //获取底部线的高度
                    mBottomLineHeight= (int) array.getDimension(attr,DEFAULT_BOTTOM_LINE_HEIGHT * getResources().getDisplayMetrics().density);
                    break;
                case R.styleable.CookTabLayout_indicatorHeight:
                    //获取Tab 高度
                    mIndicatorHeight= (int) array.getDimension(attr,DEFAULT_INDICATOR_HEIGHT * getResources().getDisplayMetrics().density);
                    break;
                case R.styleable.CookTabLayout_dividerMargin:
                     /*获取分割线的距离上线边距的距离*/
                    mDividerMargin = (int) array.getDimension(attr, DEFAULT_DIVIDER_MARGIN * getResources().getDisplayMetrics().density);
                    break;
                case R.styleable.CookTabLayout_dividerWidth:
                    //获取分割线的宽度
                    mDividerWidth= (int) array.getDimension(attr,DEFAULT_DIVIDER_WIDTH * getResources().getDisplayMetrics().density);
                    break;

            }
        }
        Log.e("dandy","color "+mBottomLineColor);
        array.recycle();
        initView();
    }
     ...
     */
    public void viewPagerChange(int position, float positionOffset) {
